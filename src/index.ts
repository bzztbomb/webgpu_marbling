...
const NUM_IMAGES = 32;
const clearValue = { r: 83 / 255, g: 102 / 255, b: 132 / 255, a: 1 };
...
let slotIndex = 0;
const gifs = new Array(NUM_IMAGES);

async function getNextImage() {
  slotIndex = (slotIndex + 1) % NUM_IMAGES;
  if (gifs[slotIndex]) {
    clearInterval(gifs[slotIndex]);
    gifs[slotIndex] = 0;
  }
  const response = await fetch("i");
  const imageName = await response.text();
  const imageUrl = `images/${encodeURI(imageName)}`;
  if (!imageUrl.endsWith("gif")) {
    loadImageToSlot(imageUrl, slotIndex);
  } else {
    gifs[slotIndex] = loadGif(imageUrl, slotIndex);
  }
}

console.log(
  "let's get ready to bumble! (remixed!): https://github.com/bzztbomb/webgpu_marbling/",
);
...
function rejoiceInError(e: Error) {
  location.href = "/v";
  throw e;
}

if (!navigator.gpu) {
  rejoiceInError(new Error("WebGPU not supported!"));
}

const adapter = await navigator.gpu.requestAdapter();
if (!adapter) {
  rejoiceInError(new Error("Unable to get adapter!"));
}

const device = await adapter.requestDevice();
if (!device) {
  rejoiceInError(new Error("Unable to get device!"));
}
...
async function loadImageToSlot(
  ...
  const image =
    img instanceof HTMLCanvasElement ? img : await loadImageBitmap(img);
  ...
  resizeUniforms[0] =
    image.height > image.width ? IMAGE_SIZE / image.height : 1;
...
const observer = new ResizeObserver((elements) => {
  ...
  uniforms[UNIFORM_ASPECT_RATIO_X] =
    canvas.height >= canvas.width ? canvas.height / canvas.width : 1.0;
  uniforms[UNIFORM_ASPECT_RATIO_Y] =
    canvas.width >= canvas.height ? canvas.width / canvas.height : 1.0;
});
try {
...
const dropPipeline = device.createRenderPipeline({
  ...
  layout: device.createPipelineLayout({
    bindGroupLayouts: [dropBindGroupLayout],
  }),
  vertex: {
...
const dropBindGroups = [1, 0].map((vb) =>
...
const earcutBindGroups = [0, 1].map((vb) =>
  device.createBindGroup({
...
const simulateBindGroups = [1, 0].map((vb) =>
  device.createBindGroup({
...
const earcutPipeline = device.createComputePipeline({
  ...
  layout: device.createPipelineLayout({
    bindGroupLayouts: [earcutBindGroupLayout],
  }),
  compute: {
...
function draw() {
  ...
  if (simulateRequired) {
    ...
    simulatePass.dispatchWorkgroups(
      Math.ceil((NUM_DROPS * NUM_DROP_VERTICES) / SIMULATE_WORKGROUP_SIZE),
    );
    simulatePass.end();
...
function loadGif(url: string, slotIndex: number): number {
  const decoder = new Decoder();
  decoder.load(url);
  return setInterval(() => {
    const nextFrame = decoder.getNextFrame();
    if (nextFrame) {
      loadImageToSlot(nextFrame, slotIndex);
    }
  }, 10);
}
