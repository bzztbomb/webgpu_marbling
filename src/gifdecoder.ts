...
export class Decoder {
  ...
  public getNextFrame(): null | HTMLCanvasElement {
    ...
    if (this.lastFrame === -1) {
      ...
      // Sometimes delay is undefined
      this.nextFrameTime = this.startTime + (this.frames[0].delay || 100);
    ...
    if (now >= this.nextFrameTime) {
      ...
      this.nextFrameTime = now + (this.frames[nextFrame].delay || 100);
  ...
  private drawFrame(frame: number): void {
    ...
    if (
      !this.frameImageData ||
      this.frameImageData.width !== dims.width ||
      this.frameImageData.height !== dims.height
    ) {
      this.patchCanvas.width = dims.width;
      this.patchCanvas.height = dims.height;
      this.frameImageData = this.patchCtx.createImageData(
        dims.width,
        dims.height,
      );
    }
